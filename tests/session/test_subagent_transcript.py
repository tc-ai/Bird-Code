...
async def test_read_subagent_transcript_roundtrip(tmp_path):
    ...
    await store.append(Message(role="assistant", content=[TextBlock(text="ok")]), is_assistant=True)
    store.close()

    turns = read_subagent_transcript(store.jsonl_path)
    assert turns
    texts = [getattr(b, "text", "") for t in turns for m in t.messages for b in m.content]
